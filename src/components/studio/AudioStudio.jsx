...
export default function AudioStudio({ mode = 'speech', active = true }) {
  ...
  const gen=useCallback(async(sp)=>{const p=sp||inputs;const tts=getAudioModelById('Qwen3-TTS'),idx=getAudioModelById('IndexTTS-2');if(!(ic?(!!p.trim()&&!!ra):!!p.trim()))return;const tid=_aid();addTask({id:tid,generationId:tid,type:mode,prompt:p.trim(),model:ic?idx.name:tts.name,status:'generating',results:null,error:null});setGc(c=>c+1);setErr(null);showToast('任务已提交','info');
    try{const params={client_generation_id:tid,model:ic?idx:tts,mode:ic?'clone':'audio',inputs:p.trim()};
      ...
      const d=await createGenerationAPI().audio(params);
      updateTask(tid,{generationId:d.generationId||tid,status:'done',results:d.results,duration:d.duration});showToast('生成完成','success');if(d.errors){setErr(`部分失败: ${d.errors.join('; ')}`);setTimeout(()=>setErr(null),10000);}
  ...
  },[inputs,lang,speaker,instr,ra,ev,et,vd,emoMode,ea,can,ic,addTask,updateTask]);
    ...
    <div className="h-full flex overflow-hidden">
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <div className="flex-1 flex flex-col min-h-0 p-4 gap-3">
          ...
          {ic&&emoMode==='text'&&<div className="flex-shrink-0"><div className="mb-1.5 px-0.5"><div className="text-[11px] font-medium text-white/55">情绪参考文本</div><div className="mt-0.5 text-[10px] leading-4 text-white/30">只用于控制朗读情绪，不替代下方朗读文本；例如欢快雀跃、低沉忧伤、愤怒激昂。</div></div><textarea value={et} onChange={e=>setEt(e.target.value)} placeholder="情绪参考文本，描述想要的情感：例如 欢快雀跃、低沉忧伤、愤怒激昂" rows={2} className="w-full bg-white/[0.03] border border-border rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/15 focus:outline-none focus:ring-1 focus:ring-primary/30 resize-none"/></div>}
          {!ic&&vd&&<div className="flex-shrink-0"><div className="mb-1.5 px-0.5"><div className="text-[11px] font-medium text-white/55">Prompt 自定义音色</div><div className="mt-0.5 text-[10px] leading-4 text-white/30">描述希望生成的音色特征、年龄感、语速、语气和质感；正文内容仍填写在下方合成文本。</div></div><textarea value={instr} onChange={e=>setInstr(e.target.value)} placeholder="Prompt自定义音色：描述你想要的音色特征，例如：温柔知性的年轻女声，语速适中，带一点磁性" rows={3} className="w-full bg-white/[0.03] border border-border rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/15 focus:outline-none focus:ring-1 focus:ring-primary/30 resize-none"/></div>}
          ...
          <PromptInput value={inputs} onChange={setInputs} label={ic?'朗读文本':'合成文本'} helpText={ic?'输入要使用参考音色朗读的文本；参考音频只决定音色，朗读内容以这里为准。':'输入要合成为语音的文本；可配合音色、语言和音色指导控制发音风格。'} placeholder={ic?'输入需要克隆朗读的文本内容...':'输入需要合成语音的文本内容。选择音色或使用音色设计，点击「生成语音」'}/>
