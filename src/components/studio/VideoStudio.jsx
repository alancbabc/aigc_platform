...
export default function VideoStudio({ mode = 'text2video', active = true }) {
  ...
  const gen=useCallback(async(submitPrompt,submitNeg)=>{const p=submitPrompt||prompt;const n=submitNeg!==undefined?submitNeg:np;if(!p.trim())return;const tid=_vid();addTask({id:tid,generationId:tid,type:mode,prompt:p.trim(),model:cm.name,status:'generating',results:null,error:null});setGc(c=>c+1);setErr(null);showToast('任务已提交','info');
    try{const ib=ri?await readFileAsBase64(ri):undefined;const ab=audio?await readFileAsBase64(audio):undefined;
      const d=await createGenerationAPI().video({client_generation_id:tid,model:cm,mode,prompt:p.trim(),image_base64:ib,negative_prompt:n.trim()||undefined,seed:seed||undefined,duration:dur,resolution:res,resolution_preset:resolutionPreset,aspect_ratio:aspectRatio,quality:cfg.sq?qual:undefined,audio_base64:ab,audio_insert_position:audio?audioPos:0});
  ...
  },[prompt,np,seed,cm,ri,dur,res,resolutionPreset,aspectRatio,qual,audio,audioPos,can,cfg.sq,addTask,updateTask]);
