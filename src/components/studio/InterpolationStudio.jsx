...
export default function InterpolationStudio() {
  const{addTask,updateTask,optimizeOpen,setOptimizeOpen,setOptimizePanel}=useTasks();const loc=useLocation();
  ...
  useEffect(()=>{if(optimizeOpen)setOptimizePanel({prompt,type:'interpolation',onApply:setPrompt});},[optimizeOpen,prompt,setOptimizePanel]);
...
const rm=(uid,i)=>{revokeObjectURL(pv[i]);setPv(p=>p.filter((_,x)=>x!==i));setFrames(p=>p.filter(f=>f.uid!==uid));};
  ...
  const gen=useCallback(async(sp,sn)=>{const p=sp||prompt;const n=sn!==undefined?sn:np;if(!p.trim()||frames.length===0)return;const tid=_iid();addTask({id:tid,generationId:tid,type:'interpolation',prompt:p.trim(),model:cm.name,status:'generating',results:null,error:null});setGc(c=>c+1);setErr(null);showToast('任务已提交','info');
    try{const b64=await Promise.all(frames.map(f=>readFileAsBase64(f.file)));const ab=audio?await readFileAsBase64(audio):undefined;
      const d=await createGenerationAPI().interpolation({model:cm,mode:'interpolation',prompt:p.trim(),frames:b64,negative_prompt:n.trim()||undefined,seed:seed||undefined,duration:dur,resolution:res,audio_base64:ab,gen_num:gn});
  ...
  },[prompt,np,seed,cm,frames,dur,res,audio,gn]);
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <div className="flex-1 flex flex-col min-h-0 p-4 gap-3">
          ...
          <div className="mt-auto flex items-center gap-2">
            ...
            <GenerateButton onClick={()=>gen()} disabled={!prompt.trim()||frames.length===0} label={prompt.trim()&&frames.length>0?'生成插帧生音视频':!prompt.trim()&&frames.length===0?'请上传关键帧图片并输入 Prompt':!prompt.trim()?'请输入 Prompt':'请上传关键帧图片'}/>
          </div>
